        ...
        <div class="lg:column-gap spacing-t grid w-full grid-cols-12">
          ...
          >
            ...
            <div class="relative origin-left overflow-hidden sm:scale-150">
              <div id="contact-btn" class="flex -translate-y-full">
                <Button label="Get in touch" :url="contactPhone" />
              </div>
            </div>
          </div>
          ...
          >
            ...
            <img
              id="profile-img"
              :src="profile"
              alt="Kshitij Tripathi profile"
              class="size-full scale-90 rounded-lg object-cover object-center mix-blend-multiply grayscale"
            />
          </div>
...
<script setup lang="ts">
  ...
  import { contactPhone, heroText } from '@/data';

  const whoAmIPlain = heroText;
  const whoAmI = ref(whoAmIPlain);
