<template>
  ...
  >
    ...
    <template v-if="!isSmallScreen && people.length">
      <div
...
<script setup lang="ts">
  ...
  // data
  const index = ref(0);
  // TODO: add real testimonials here, then re-enable <People /> in App.vue and
  // the Testimonials entry in src/data.ts. Any number of entries works.
  const people: {
    quote: string;
    author: string;
    position: string;
